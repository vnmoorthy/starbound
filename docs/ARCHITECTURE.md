...
The 3D renderer caps representative collectors at 2,400 and display pixel ratio at 2, targets roughly 30 frames/s, pauses on user request and respects reduced-motion preference. Rendered sizes, stellar appearance and time are illustrative. All orbits are circular samples; there is no N-body integration, collision avoidance or heat simulation in the renderer. A WebGL failure uses an interactive 500-collector canvas projection; drag, zoom, camera presets, pause and keyboard input remain available. The default artwork viewer supports pan and zoom and is labeled as a concept. Native GPU rendering must be evaluated on target devices before making performance or photorealism claims.
